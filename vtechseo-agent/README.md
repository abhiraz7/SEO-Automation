...
2. On activation, the plugin generates a random 64-character API token and
   ...
   `vtseo_api_token`). This token never leaves your server until you copy
   it into the VtechSEO dashboard yourself.
3. Open **VtechSEO Agent** in your WordPress admin sidebar. Copy the
...
4. From then on, VtechSEO talks to your site by sending that token in an
   ...
   `https://yoursite.com/wp-json/vtseo/v1/tool` (or the fuller
   `/wp-json/vtseo/v1/mcp` endpoint for MCP-compatible clients). Every
