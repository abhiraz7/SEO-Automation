...
class VTSEO_MCP {
    ...
    private static function open_sse_stream(): void {
        ...
        VTSEO_Logger::log( 'info', 'MCP SSE channel closed' );
        exit;
    }
    ...
    private static function handle_initialize( $id, array $params ): array {
        ...
        VTSEO_Logger::log( 'info', "MCP session initialized by: {$client}" );
    ...
    private static function handle_tools_call( $id, array $params ): array {
        ...
        VTSEO_Logger::log( 'info', "MCP tools/call: {$name}" );

        try {
            $result = VTSEO_Router::call_tool( $name, $arguments, $allowed );
            return self::make_result( $id, [
                'content' => [ [
                    'type' => 'text',
            ...
            VTSEO_Logger::log( 'error', "MCP tools/call '{$name}': " . $e->getMessage() );
            return self::make_result( $id, [
