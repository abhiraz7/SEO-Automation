<?php
/**
 * Handles authentication for every VtechSEO Agent request.
 * Supports two methods:
 *  1. Bearer token  — Authorization: Bearer <vtseo_api_token>
...
class VTSEO_Auth {
    ...
    public static function permission_callback( WP_REST_Request $request ) {
        if ( ! get_option( 'vtseo_enabled', '1' ) ) {
            return new WP_Error( 'vtseo_disabled', 'VtechSEO Agent is disabled.', [ 'status' => 503 ] );
        }

        $ip = sanitize_text_field( $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0' );

        if ( self::is_rate_limited( $ip ) ) {
            VTSEO_Logger::log( 'warn', "Rate limit hit from {$ip}" );
            return new WP_Error( 'vtseo_rate_limited', 'Too many failed attempts. Try again later.', [ 'status' => 429 ] );
        }

        $header = $request->get_header( 'Authorization' );

        if ( empty( $header ) ) {
            return new WP_Error( 'vtseo_no_auth', 'Authorization header missing. Use Bearer token or Basic Auth (Application Password).', [ 'status' => 401 ] );
        }

        /* ── Bearer token (custom API token) ──────────────── */
        if ( strpos( $header, 'Bearer ' ) === 0 ) {
            $stored = get_option( 'vtseo_api_token', '' );
            if ( empty( $stored ) ) {
                return new WP_Error( 'vtseo_no_token', 'No API token configured.', [ 'status' => 500 ] );
            }
            $token = substr( $header, 7 );
            if ( ! hash_equals( $stored, $token ) ) {
                self::record_failed_attempt( $ip );
                VTSEO_Logger::log( 'warn', "Failed Bearer token attempt from {$ip}" );
                return new WP_Error( 'vtseo_forbidden', 'Invalid token.', [ 'status' => 403 ] );
            }
        ...
        if ( strpos( $header, 'Basic ' ) === 0 ) {
            ...
            self::record_failed_attempt( $ip );
            VTSEO_Logger::log( 'warn', "Failed Basic Auth attempt from {$ip}" );
            return new WP_Error( 'vtseo_forbidden', 'Invalid Application Password or insufficient permissions.', [ 'status' => 403 ] );
        }

        return new WP_Error( 'vtseo_bad_auth', 'Expected Bearer token or Basic Auth (Application Password).', [ 'status' => 401 ] );
    }

    /** Regenerate Bearer token and return new value. */
    public static function regenerate_token(): string {
        $token = bin2hex( random_bytes( 32 ) );
        update_option( 'vtseo_api_token', $token );
        VTSEO_Logger::log( 'info', 'API token regenerated.' );
        return $token;
    }
    ...
    public static function create_application_password(): array {
        ...
        update_option( 'vtseo_app_username', $user->user_login );
        delete_option( 'vtseo_app_password' ); // Remove any legacy plaintext value.

        VTSEO_Logger::log( 'info', "Application Password created for: {$user->user_login}" );
