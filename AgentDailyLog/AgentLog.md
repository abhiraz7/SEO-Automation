...
## 2026-08-08 — Session: First AWS deployment + CI/CD pipeline (teaching mode)

### Done
- User's goal was explicitly pedagogical: learn AWS/Docker/CI-CD hands-on
  while shipping a real deployment, framed as "solo automation engineer
  becoming more than a developer." Worked as mentor throughout -- user
  typed/reviewed most files themselves; a few pieces (final workflow YAML,
  most AWS CLI commands) were done directly after the user asked to move
  faster partway through.
- **AWS account setup**: CloudWatch billing alarm, AWS Budgets (a "Zero
  Spend Budget" and an $80 "Vtechys Budget"), and a Budget Action that
  auto-stops the EC2 instance at 90% of $80 real usage -- all 4 layers
  verified via CLI (`aws budgets describe-budgets` /
  `describe-budget-actions-for-budget`), not just clicked through.
  Discovered mid-session the account has $100 in promotional credits
  (expires 2027-02-08), which is why the zero-spend budget alone wasn't
  sufficient -- credits can mask real spend from a naive charges-only
  alarm.
- **IAM**: created `abhinav-admin` (personal, `AdministratorAccess`, no
  console login, CLI-only) and `github-actions-deploy` (scoped to
  `ssm:SendCommand`/`ssm:GetCommandInvocation` on exactly one instance) --
  least-privilege split between "my own broad access" and "a third-party
  system's narrow access." A `github-actions-deploy` access key got
  accidentally pasted into chat once; rotated immediately (old key
  deleted, fresh one generated by the user directly, never relayed
  through the assistant).
- **EC2**: launched `i-0699040f972b9868b` (t3.micro, free-tier), access
  via **SSM Session Manager only** -- no SSH, no key pair, no open port
  22, chosen specifically because the user's home IP is dynamic. Security
  group allows 80/443 only. Standard credit specification (not
  Unlimited) and basic (not detailed) CloudWatch monitoring, both to
  avoid the two "Additional charges apply" traps on the launch screen.
- **Containerized the app**: `Dockerfile` (python:3.12-slim +
  playwright/crawl4ai's system libs), `.dockerignore` (caught `.env`
  almost getting baked into the image layer before this existed --
  real secret-leak risk avoided), `docker-compose.yml` (bind-mounts
  `data/seo_automation.db` for persistence across rebuilds; maps host
  80 -> container 8000 to match the security group).
- **CI/CD**: GitHub Actions workflow on push to `main` -> `aws
  ssm send-command` on the EC2 box (git pull, rewrite `.env` from
  GitHub Secrets, `docker compose up -d --build`). No SSH key anywhere
  in the pipeline -- same SSM channel used for manual access.

### Bugs hit and fixed, in order (worth remembering for next deploy)
1. SQLite bind-mount pointed at a file that didn't exist yet on the
   host -> Docker silently created a directory instead of a file at
   that path -> `sqlite3.OperationalError: unable to open database
   file`. Hit this **twice** (once locally, once again on the EC2 box
   on the very first CI deploy) before baking a guard into `deploy.yml`
   itself (checks for a stray directory, removes it, touches an empty
   file, chowns it, every deploy).
2. First workflow version tried to write `.env` via a heredoc nested
   three layers deep (YAML `run:` block -> JSON SSM `commands` array ->
   bash heredoc). The `\n` meant to be newlines survived as literal `n`
   characters, corrupting the heredoc terminator -- and printed all
   five secret values in plaintext into AWS's own SSM command history
   in the process. Fixed by building the SSM command list as real JSON
   via a Python step on the runner (proper escaping) and writing `.env`
   with one `echo ... >> .env` per key instead of a heredoc.
3. `docker compose build` failed on the box: buildx too old. Fixed by
   installing a current `docker-buildx` binary via SSM (first attempt
   grabbed a guessed version tag that 404'd silently -- 9-byte "binary"
   -- fixed by querying GitHub's release API for the actual latest tag
   instead of guessing).
4. `docker-compose.yml` initially mapped `8000:8000`; the security
   group only allows 80/443, so the app was unreachable (connection
   timeout, not refused) until the mapping was changed to `80:8000`.
- End state: `http://54.80.253.215` returns HTTP 200, deployed via a
  real `git push` -> GitHub Actions -> SSM -> Docker rebuild, no manual
  steps, verified by curl not just "the workflow said success."

### Known gaps, explicitly not done this session
- **Postgres migration (task-list 6.1)** -- production is still SQLite
  on the EC2 box's disk, single point of failure, no backup strategy.
- **Task-list 6.2 as originally written** (NSSM + Caddy + `deploy.ps1`,
  a Windows-service-based plan) was not followed -- a different,
  Docker/Linux/GitHub-Actions-based deployment was built instead, which
  achieves the same live/auto-restarting outcome but is not the
  documented plan. Worth reconciling the task list with what actually
  got built.
- No monitoring/alarms for the instance or container crashing (only
  billing is alarmed on); `restart: unless-stopped` is configured but
  was never tested against a real crash.
- No domain name / HTTPS -- app is plain HTTP on a bare IP, openly
  reachable from anywhere (security group is `0.0.0.0/0` on port 80),
  by explicit user choice for now ("this is for testing").
- User asked about sharing the live link with their team; confirmed
  it's already reachable by anyone, not just the team, given the open
  security group -- no restriction added, user's call to make later.

### Next
- User's call on: Postgres migration, monitoring/auto-recovery, db
  backup, and domain+HTTPS, all flagged but explicitly deferred
  ("no need to setup anything" at end of session).
- If task-list Phase 6 gets revisited, reconcile 6.2's original
